-- Based on the theta theme provided with alpha: https://github.com/goolord/alpha-nvim/blob/main/lua/alpha/themes/theta.lua
...
local function sessions()
	local options = {}
	for _, v in ipairs(session_manager.get_sessions()) do
		local ico = " "
		local fname = vim.fn.fnamemodify(v.dir.filename, ":~")
		fname = plenary_path.new(fname):shorten(1, { -2, -1 })
		table.insert(options, {
			type = "button",
			val = ico .. " " .. fname,
			on_press = function ()
				session_manager.load_session(v.filename, false)
			end,
			opts = { position = "center", cursor = 3, width = 50, align_shortcut = "right", hl_shortcut = "Keyword", shortcut = "" },
		})
	end
...
local buttons = {
	...
	val = {
		...
		dashboard.button("SPC s", "󰙅  Browse files"),
		dashboard.button("SPC r g", "󰈞  Find file"),
		dashboard.button("SPC r r", "󰊄  Live grep"),
		dashboard.button("c", "  Configuration", "<cmd>cd "..vim.fn.stdpath('config').."<CR><cmd>NvimTreeOpen<CR>"),
